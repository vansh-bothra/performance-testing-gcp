...
public class ApiFlowV3Main {
    ...
    private void runWaveMode(List<String> uidPool) {
        ...
        WaveExecutor executor = new WaveExecutor(rps, duration, randomUid, uidPool, title, verbose, true);
    ...
    private void printSingleResult(Map<String, Object> result, double totalTimeMs) {
        ...
        if (!Boolean.TRUE.equals(result.get("success"))) {
            ...
            double datePickerMs = (Double) s1.getOrDefault("date_picker_latency_ms", 0.0);
            double cdnMs = (Double) s1.getOrDefault("cdn_latency_ms", 0.0);
            System.out.printf("    Step 1 (date-picker + CDN): %.1f ms (HTML: %.1f ms, CDN: %.1f ms)%n",
                    s1.getOrDefault("latency_ms", 0.0), datePickerMs, cdnMs);
